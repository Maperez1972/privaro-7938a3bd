    ...
    <section id="solution" className="py-28 px-6 relative">
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.7, delay: 0.3 }} className="mt-10">
          <img src={lang === "es" ? solutionImageEs : solutionImageEn} alt={lang === "en" ? "Diagram showing the bidirectional flow: enterprise data anonymized by the Privaro layer before reaching the AI model, and the model response scanned for PII on the way back to the protected user" : "Diagrama del flujo bidireccional: los datos empresariales se anonimizan en la capa Privaro antes de llegar al modelo de IA, y la respuesta del modelo se escanea en busca de PII antes de llegar al usuario protegido"} className="w-full max-w-3xl mx-auto rounded-xl border border-border" loading="lazy" decoding="async" width={1344} height={768} />
        </motion.div>
